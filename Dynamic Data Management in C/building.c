...
// Dynamically allocate memory for the ghosts list
  b->ghosts = (GhostListType *)malloc(sizeof(GhostListType));
  if (b->ghosts != NULL) {
    initGhostList(b->ghosts);
  }

// Dynamically allocate memory for the rooms array
  b->rooms = (RoomArrayType *)malloc(sizeof(RoomArrayType));
  if (b->rooms != NULL) {
...
void cleanupBuilding(BuildingType *b){
 // Clean up the room data
  ...
  // Clean up the ghost data
  cleanupGhostData(b->ghosts);
  cleanupGhostList(b->ghosts);

  // Deallocate the memory for the rooms array and the ghosts list
  free(b->rooms);
  b->rooms = NULL;
  free(b->ghosts);
  b->ghosts = NULL;
